...
###  문제 설명
영화 리뷰 정보를 관리하기 위한 `MovieReview` 클래스를 만들어보자.

###  요구사항
1. `MovieReview` 클래스는 다음 멤버 변수를 포함해야 함
  - 영화 제목 (`title`)
  - 리뷰 내용 (`review`)
2. `MovieReviewMain` 클래스에서 영화 리뷰 정보를 선언하고 출력할 것
...
기존 문제에서 **배열을 도입**하여 여러 영화 리뷰를 한 번에 관리하고,
출력 시 **for문을 사용**해 깔끔하게 출력하자.
...
**배열과 향상된 for문 덕분에**
코드가 훨씬 간결해지고, 새로운 영화 리뷰가 추가되어도 반복문만 돌리면 자동으로 출력됨.
...
온라인 상점의 상품 주문 정보를 관리하는 시스템을 만들어보자.

---

###  요구사항

1. `ProductOrder` 클래스 정의

  * 상품명 (`productName`)
  * 가격 (`price`)
  * 수량 (`quantity`)
2. `ProductOrderMain` 클래스에서 여러 상품 주문을 배열로 관리
3. 각 상품의 주문 정보와 **총 결제 금액**을 출력
...
| 개념             | 설명                          |
| -------------- | --------------------------- |
| **클래스(Class)** | 객체의 설계도, 데이터 구조를 정의         |
| **객체(Object)** | 클래스의 실제 인스턴스                |
| **배열(Array)**  | 같은 타입의 데이터를 연속적으로 저장        |
| **향상된 for문**   | 인덱스 없이 배열 요소를 순회하는 간결한 문법   |
| **총합 계산 로직**   | 반복문을 통해 price × quantity 합산 |
